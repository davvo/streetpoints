  ...
  segments.all().forEach(item => {
    segments.search(item).forEach(other => {
      ...
      if (result.type === 'intersecting') {
        ...
        const tag = item.tag || other.tag
        points.push({ lng, lat, name, tag })
      }
    })
  })
...
const extractPoints = geojson => {
  ...
  const addPoint = (coords, name, tag) => {
  ...
  // Find all crossroads
  findIntersections(geojson).forEach(({ lng, lat, name, tag }) => addPoint([lng, lat], name, tag))
  ...
  geojson.features.forEach(feature => {
    ...
    const name = feature.properties.name
    const tag = feature.properties.tag
    addPoint(coords[0], name, tag)
    addPoint(coords[coords.length - 1], name, tag)
  })
